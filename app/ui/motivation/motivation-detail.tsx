...
export default function MotivationDetail(params: { motivation: MotivationModel | undefined }) {
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="p-2">
        <h3 className="">Doporučil vám někto přidat se k programu? Pokud ano, kto?</h3>
        <div>{params.motivation.who}</div>
      </div>
      <div className="p-2 bg-amber-100 rounded-md">
        <h3 className="">Co byste chtěli jako ambasador dosáhnout?</h3>
        <div>{params.motivation.goals}</div>
      </div>
      <div className="p-2">
        <h3 className="">Jakých akcí byste se chtěli převážně zúčastňovat?</h3>
        <div>{params.motivation.preferred_events}</div>
      </div>
      <div className="p-2 bg-amber-100 rounded-md">
